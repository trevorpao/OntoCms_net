...
### 對外回應規則
- 不要因為單一文件寫了 `done` 或其他 stage 名稱，就直接對外宣稱「目前 stage 是 done」或其他 stage 結論。
- 對外若要宣稱某個 spec 已沒有待辦，必須同時滿足三個條件：`plan.md` 已無可執行項、`check.md` 已無待辦且驗收項已全部打勾、`idea.md` / `plan.md` / `check.md` 三者一致。
- 在上述條件未同時成立前，優先回應「目前 spec 尚有哪些待辦事項」與「下一步是什麼」，而不是主動輸出 stage 名稱作為結論。
...
- 不要把單一文件中的 stage 標示直接上升為對外結論；若要回應 spec 已無待辦，仍需回到 `plan.md`、`check.md`、`idea.md` 三者交叉確認。
